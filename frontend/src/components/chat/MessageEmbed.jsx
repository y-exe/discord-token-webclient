...
const MessageEmbed = ({ embed, onImageClick }) => {
  ...
  return (
    <div className="border-l-4 bg-[#1e1f22] rounded-r p-3 mt-2 max-w-[520px] grid gap-2" style={{ borderColor: embed.color || '#1e1f22' }}>
      <div className="grid gap-1">
        {embed.provider && <div className="text-xs text-discord-muted">{embed.provider.name}</div>}
        {embed.author && (
          <div className="flex items-center gap-2 text-sm font-bold text-white">
            {embed.author.iconURL && <img src={getProxyUrl(embed.author.iconURL)} className="w-6 h-6 rounded-full" />}
      ...
      {(embed.footer || embed.timestamp) && (
        <div className="text-xs text-discord-muted flex items-center gap-2 mt-1">
          {embed.footer?.iconURL && <img src={getProxyUrl(embed.footer.iconURL)} className="w-4 h-4 rounded-full" />}
          <span>{embed.footer?.text}{embed.footer && embed.timestamp && " • "}{embed.timestamp && new Date(embed.timestamp).toLocaleString()}</span>
        </div>
      )}
    </div>
  );
